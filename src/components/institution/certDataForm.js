	...
	const getAllCerts = () => {
		...
		serials.forEach((sno) => {
			...
			if (isSpreadsheet) {
				...
				address = document.getElementById(
					"address-input-for-row-" + sno
				).value;
				email = document.getElementById(
					"email-input-for-row-" + sno
				).value;
			}
	...
	return (
		<div className="certIssueFormContainer">
			...
			<div className="buttonContainer">
				...
				<button onClick={() => setOpen(false)}>
					{t("Institutions.certIssue.issueAll")}
					<DescriptionIcon
			...
			{isSpreadsheet ? (
				<button
					...
					style={{ margin: "auto" }}>
					{t("Institutions.certIssue.closeSpreadsheet")}
				...
				<>
					<h3>{t("Institutions.certIssue.alreadyExcelsheet")}</h3>
					<button
						onClick={() => setIsSpreadsheet(true)}
						style={{ margin: "auto" }}>
						{t("Institutions.certIssue.copypagesheet")}
						<BorderAllIcon
							sx={{ fontSize: 20, marginLeft: "10px" }}
						/>
					</button>
			...
			{isSpreadsheet ? (
				...
				<>
					<div
						...
						<h3>{t("Institutions.certIssue.sno")}</h3>
						{certData["variable1"] !== "" && (
							<h3>{certData["variable1"]}</h3>
						)}
						{certData["variable2"] !== "" && (
							<h3>{certData["variable2"]}</h3>
						)}
						<h3>{t("Institutions.certIssue.recipientAddress")}</h3>
						<h3>{t("Institutions.certIssue.recipientEmail")}</h3>
					</div>
			...
			<button
				...
				{t("Institutions.certIssue.issueAllCertificates")}
	...
	return (
		<>
			<div
				...
				<h3>{t("Institutions.certIssue.sno")}</h3>
				<h3>{t("Institutions.certIssue.recipient")}</h3>
				<h3>{t("Institutions.certIssue.status")}</h3>
				<h3>{t("Institutions.certIssue.tokenId")}</h3>
				<h3>{t("Institutions.certIssue.download")}</h3>
			</div>
			{res.map((cert, index) => {
				return (
