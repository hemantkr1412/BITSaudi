...
const CertIssue = ({ category, setCategory, setIsBatchCreator }) => {
	...
	const uploadFile = () => {
		if (user.userData.nft_quota === 0) {
			alert(t("dNFT.alert.upgradePlan"));
			navigate("/subscription");
			return;
		} else if (certNumber > parseInt(user.userData.nft_quota)) {
			alert(t("dNFT.alert.exceeded"));
			navigate("/subscription");
			return;
		}
		setIsLoading(true);
		...
		dNFtForStudent({
			...
			.then((res) => {
				if (res === "issued") {
					setStatus("Dynamic NFT Issued Successfully");
			...
			<div style={{ width: "500px" }}>
				...
				<div
					...
					<QRCode
						...
						style={{
							width:
								(parseFloat(imageWidth) * 0.1).toString() +
								"px",
							height:
								(parseFloat(imageWidth) * 0.1).toString() +
								"px",
