	...
	const MainPage = () => {
		...
		return (
			<div
				...
				{category === "Create New Batch" &&
					(isBatchCreator ? (
						<CertIssue setCategory={setCategory} />
					) : (
						<div
							style={{
								width: "100%",
				...
				{category === "Update Batch" && (
					<div>
						...
						{selectedBatch.name === "" ? (
							...
							<div>
								<h4>
									{t("dNFT.BatchName")} : {selectedBatch.name}
								</h4>
								<h4>
									{t("dNFT.BatchDescription")} :{" "}
								...
								<div
									...
									<QRCode
										...
										value={
											"https://bitmemoirlatam.com/#/verify/"
										}
									/>
								</div>
	...
	return (
		<div>
			<div
				...
				<div>{t("Institutions.certIssue.sno")}</div>
			...
			{students?.students?.map((student, index) => (
				<div
					...
					<div>
						{student.is_minted ? (
							<OpenInNewIcon
								onClick={() => window.open(student.nft_image)}
							/>
						) : (
							"-"
						)}
